...
export function findMatchingExample(source, examples) {
  const normalized = normalizeSource(source);
  return examples.find((example) => normalizeSource(example.source) === normalized) ?? null;
}
...
export function buildShareFragment({ source, runtimeHint, examples }) {
  ...
  if (matchingExample) {
    ...
    params.set('v', APP_CONFIG.shareVersion);
